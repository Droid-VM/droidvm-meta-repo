...
# maintained successor of the whitebelyash/mesa-tu8 lineage (archived 2026-04).
# Its build_droidvm.sh clones UPSTREAM mesa at the
...
#
# Local driver work does NOT live in the build tree: every rebuild resets the mesa tree
# and re-applies the patch stack, so changes belong in the fork's patches/ (edit patch,
# push, bump pin). The mesa checkout under turnip/banners-turnip is disposable.
#
...
BANNERS_PIN=${BANNERS_PIN:-dcc9e3c639727210bdaf20dd33c79dd8182f0647}
...
if [ ! -d "$MESA_DIR" ]; then
    echo ">>> building turnip from $BANNERS_URL (pinned mesa, ~5 min with a reused NDK)"
