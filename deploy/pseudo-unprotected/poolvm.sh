...
set -u
PHONE=${PHONE:-5568}
A="adb -s 10.53.12.1:$PHONE"
ROOT=/root/Documents/DroidVM_meta
OUT=$ROOT/crosvm_out
...
eui() { printf '2001:db8:1:1:%02x%02x:%02xff:fe%02x:%02x%02x\n' \
...
EOF
    ...
    setsid bash -c "adb -s 10.53.12.1:$PHONE shell su -c 'tail -n +1 -F $DBG/crosvm.log'" > "$LOG" 2>&1 </dev/null &
