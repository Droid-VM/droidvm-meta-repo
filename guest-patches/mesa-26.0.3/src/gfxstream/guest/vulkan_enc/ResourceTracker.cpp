...
void ResourceTracker::on_vkGetPhysicalDeviceFeatures2(void*, VkPhysicalDevice,
    ...
    if (pFeatures) {
        ...
        // DroidVM: the generated encoder does not marshal Robustness2 feature
        // structs, so the query never reaches the host driver. The host
        // (VkDecoderGlobalState) force-enables VK_EXT_robustness2 with the
        // real turnip feature values at device creation, so report the
        // capability here (a830 turnip supports all three).
        VkPhysicalDeviceRobustness2FeaturesEXT* robustness2FeaturesEXT =
            vk_find_struct(pFeatures, PHYSICAL_DEVICE_ROBUSTNESS_2_FEATURES_EXT);
        if (robustness2FeaturesEXT) {
            robustness2FeaturesEXT->robustBufferAccess2 = VK_TRUE;
            robustness2FeaturesEXT->robustImageAccess2 = VK_TRUE;
...
void ResourceTracker::on_vkGetPhysicalDeviceProperties2(void* context,
    ...
    *pProperties = localProps;
    ...
    mesa_logw("DROIDVM-DRM-Q: instance=%p drmProps=%p", (void*)instance, (void*)drmProps);
    if (instance && drmProps) {
        VirtGpuDrmInfo drmInfo;
        if (instance->getDrmInfo(&drmInfo)) {
            mesa_logw("DROIDVM-DRM: hasPrim=%d prim=%lld:%lld hasRend=%d rend=%lld:%lld",
                      (int)drmInfo.hasPrimary, (long long)drmInfo.primaryMajor, (long long)drmInfo.primaryMinor,
                      (int)drmInfo.hasRender, (long long)drmInfo.renderMajor, (long long)drmInfo.renderMinor);
...
#define _RETURN_FAILURE_WITH_DEVICE_MEMORY_REPORT(result)                                      \
    ...
    if (importDmabuf) {
        ...
        mesa_logw("DROIDVM-DIAG alloc: exportDmabuf=1 dedAllocInfo=%p hasDedImg=%d hasDedBuf=%d hostVisible=%d size=%llu",
                  (void*)dedicatedAllocInfoPtr, (int)hasDedicatedImage, (int)hasDedicatedBuffer,
                  (int)requestedMemoryIsHostVisible, (unsigned long long)finalAllocInfo.allocationSize);
        ...
        if (hasDedicatedImage) {
            ...
            if (mCaps.vulkanCapset.alwaysBlob) {
                ...
                {
                    // DROIDVM: host returns memreq.size=0 for external images; derive a valid
                    // size from the image dimensions (host colorBuffer is created at these dims
                    // via create3d). 4 bpp covers all 8-bit-RGBA swapchain formats.
                    uint64_t droidvm_sz = finalAllocInfo.allocationSize;
                    if (!droidvm_sz)
                        droidvm_sz = (uint64_t)imageCreateInfo.extent.width *
                                     imageCreateInfo.extent.height * 4ull *
                                     (imageCreateInfo.arrayLayers ? imageCreateInfo.arrayLayers : 1);
                    finalAllocInfo.allocationSize = droidvm_sz;
                    createBlob.size = droidvm_sz;
                    mesa_logw("DROIDVM-FIX: image size derived=%llu (w=%u h=%u layers=%u)",
                              (unsigned long long)droidvm_sz, imageCreateInfo.extent.width,
            ...
            // DROIDVM: non-dedicated exportable host-visible memory (e.g. wsi staging) -> create a
            // generic linear buffer resource so on_vkGetMemoryFdKHR can export it.
            uint32_t virglFormat = VIRGL_FORMAT_R8_UNORM;
            const uint32_t target = PIPE_BUFFER;
            uint32_t bind = VIRGL_BIND_LINEAR;
            uint32_t width = finalAllocInfo.allocationSize ? finalAllocInfo.allocationSize : 4096;
            uint32_t height = 1;
            mesa_logw("DROIDVM-FIX: non-dedicated exportable -> buffer resource size=%u", width);
            if (mCaps.vulkanCapset.alwaysBlob) {
                struct gfxstreamResourceCreate3d create3d = {};
                struct VirtGpuExecBuffer exec = {};
                struct gfxstreamPlaceholderCommandVk placeholderCmd = {};
                struct VirtGpuCreateBlob createBlob = {};
                create3d.hdr.opCode = GFXSTREAM_RESOURCE_CREATE_3D;
                create3d.bind = bind; create3d.target = target; create3d.format = virglFormat;
                create3d.width = width; create3d.height = height; create3d.blobId = ++mAtomicId;
                createBlob.blobCmd = reinterpret_cast<uint8_t*>(&create3d);
                createBlob.blobCmdSize = sizeof(create3d);
                createBlob.blobMem = kBlobMemHost3d;
                createBlob.flags = kBlobFlagShareable | kBlobFlagCrossDevice;
                ...
                exec.command = static_cast<void*>(&placeholderCmd);
                exec.command_size = sizeof(placeholderCmd);
                exec.flags = kRingIdx; exec.ring_idx = 1;
                if (instance->execBuffer(exec, bufferBlob.get())) return VK_ERROR_OUT_OF_HOST_MEMORY;
                bufferBlob->wait();
            } else {
                bufferBlob = instance->createResource(width, height, width, width * height,
                                                      virglFormat, target, bind);
                if (!bufferBlob) return VK_ERROR_OUT_OF_DEVICE_MEMORY;
                if (bufferBlob->wait()) return VK_ERROR_OUT_OF_DEVICE_MEMORY;
            }
            hasDedicatedBuffer = true;  // route import via importBufferInfo
        }
    }
...
void ResourceTracker::transformImageMemoryRequirements2ForGuest(VkImage image,
    ...
    VkMemoryDedicatedRequirements* dbg_dr = vk_find_struct(reqs2, MEMORY_DEDICATED_REQUIREMENTS);
    mesa_logw("DROIDVM-DIAG memreq2: image=%p external=%d handleTypes=0x%x dedReqsInChain=%p",
              (void*)image, (int)info.external,
              (unsigned)info.externalCreateInfo.handleTypes, (void*)dbg_dr);
