...
tu_free_zombie_vma_locked(struct tu_device *dev, bool wait)
{
   ...
   while (u_vector_length(&vdev->zombie_vmas_stage_2) > 0) {
      ...
      /* TU_NO_VA_REUSE: leak the VA range (A/B probe for the
       * reuse-before-quiescent corruption — a fresh BO bound over a recycled
       * VA replaces pages under any still-executing reference). */
      if (!getenv("TU_NO_VA_REUSE"))
