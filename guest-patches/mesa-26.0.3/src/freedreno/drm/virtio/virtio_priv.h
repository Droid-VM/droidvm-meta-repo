...
struct virtio_device {
   ...
   /* Arena v2: one blessed page pool; non-shared BOs suballocate from it and
    * alias its single guest mapping (no per-BO virtio map / GuestAccept). */
   struct fd_bo *arena_bo;
   uint8_t *arena_map;
   uint64_t arena_size;
   struct util_vma_heap arena_pool;
   simple_mtx_t arena_lock;
};
...
struct virtio_bo {
   struct fd_bo base;
   /* Arena v2: nonzero len = paged arena BO backed by [arena_off, +len). */
   uint64_t arena_off;
   uint64_t arena_len;
