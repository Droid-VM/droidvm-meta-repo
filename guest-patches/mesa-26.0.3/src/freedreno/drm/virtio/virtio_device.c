...
virtio_device_new(int fd, drmVersionPtr version)
{
   ...
   /* DroidVM KGSL nctx: prefer the per-context VA slice from GET_PARAM over
    * the global capset range (see tu_knl_drm_virtio.cc). */
   {
      uint64_t vs = 0, vz = 0;
      struct msm_ccmd_ioctl_simple_req *req;
      struct msm_ccmd_ioctl_simple_rsp *rsp;
      unsigned req_len = sizeof(*req) + sizeof(struct drm_msm_param);
      req = (struct msm_ccmd_ioctl_simple_req *)malloc(req_len);
      req->hdr = MSM_CCMD(IOCTL_SIMPLE, req_len);
      req->cmd = DRM_IOCTL_MSM_GET_PARAM;
      struct drm_msm_param *p = (struct drm_msm_param *)req->payload;
      for (int which = 0; which < 2; which++) {
         memset(p, 0, sizeof(*p));
         p->pipe = MSM_PIPE_3D0;
         p->param = which ? MSM_PARAM_VA_SIZE : MSM_PARAM_VA_START;
         rsp = (struct msm_ccmd_ioctl_simple_rsp *)
            vdrm_alloc_rsp(virtio_dev->vdrm, &req->hdr,
                           sizeof(*rsp) + sizeof(*p));
         if (vdrm_send_req(virtio_dev->vdrm, &req->hdr, true) || rsp->ret)
            break;
         uint64_t v = ((struct drm_msm_param *)rsp->payload)->value;
         if (which) vz = v; else vs = v;
      }
      free(req);
      if (vs && vz) {
   ...
   /* Arena v2 blessing: allocate the host page pool once and map it once —
    * the session's only GuestAccept.  Gated by VDRM_ARENA (MB) until the
    * capset advertises host support. */
   if (getenv("VDRM_ARENA")) {
      uint64_t pool_mb = strtoull(getenv("VDRM_ARENA"), NULL, 0);
      if (!pool_mb)
         pool_mb = 1024;
      uint64_t pool_size = pool_mb << 20;
      ...
      uint32_t handle = vdrm_bo_create(virtio_dev->vdrm, pool_size,
                                       VIRTGPU_BLOB_FLAG_USE_MAPPABLE,
                                       req.blob_id, &req.hdr);
      if (handle) {
         virtio_dev->arena_map =
            vdrm_bo_map(virtio_dev->vdrm, handle, pool_size, NULL);
         if (virtio_dev->arena_map) {
            virtio_dev->arena_size = pool_size;
            /* offset 0 is the failure sentinel of util_vma_heap: waste the
             * first page. */
            util_vma_heap_init(&virtio_dev->arena_pool, 4096,
                               pool_size - 4096);
            simple_mtx_init(&virtio_dev->arena_lock, mtx_plain);
            INFO_MSG("arena blessed: %"PRIu64" MB", pool_mb);
         } else {
            ERROR_MSG("arena map failed; falling back to per-BO path");
         }
      } else {
         ERROR_MSG("arena blessing failed; falling back to per-BO path");
      }
   }
