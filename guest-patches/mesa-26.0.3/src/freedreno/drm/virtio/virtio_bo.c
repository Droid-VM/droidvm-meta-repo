...
virtio_bo_mmap(struct fd_bo *bo)
{
   ...
   /* Arena v2: CPU access aliases the pool's single mapping. */
   struct virtio_device *vdev = to_virtio_device(bo->dev);
   if (virtio_bo->arena_len)
      return vdev->arena_map + virtio_bo->arena_off;
...
virtio_bo_finalize(struct fd_bo *bo)
{
   ...
   /* Arena v2: return the pool backing.  Safe here without extra fencing —
    * the per-pipe retire_queue holds a ref on every submit (and thus its
    * BOs) until the out-fence signals, so finalize runs post-retire. */
   if (virtio_bo->arena_len) {
      struct virtio_device *vdev = to_virtio_device(bo->dev);
      simple_mtx_lock(&vdev->arena_lock);
      util_vma_heap_free(&vdev->arena_pool, virtio_bo->arena_off,
                         virtio_bo->arena_len);
      simple_mtx_unlock(&vdev->arena_lock);
      virtio_bo->arena_len = 0;
      /* the mmap belongs to the arena, don't let core munmap it */
      bo->map = NULL;
   }
}
...
virtio_bo_new(struct fd_device *dev, uint32_t size, uint32_t flags)
{
   ...
   /* Arena v2 fast path: suballocate the backing from the blessed pool and
    * describe it to the host as a (single, for now) run.  Shared/scanout BOs
    * and pool-exhaustion fall through to the legacy per-BO path. */
   if (virtio_dev->arena_size && !(flags & (FD_BO_SHARED | FD_BO_SCANOUT))) {
      uint64_t asize = ALIGN_POT((uint64_t)size, 4096);
      simple_mtx_lock(&virtio_dev->arena_lock);
      uint64_t aoff = util_vma_heap_alloc(&virtio_dev->arena_pool, asize,
                                          4096);
      simple_mtx_unlock(&virtio_dev->arena_lock);
      if (aoff) {
         struct {
            struct msm_ccmd_gem_new_req req;
            uint32_t nr_runs;
            uint32_t pad;
            struct msm_gem_new_run run;
         } areq = {
            .req = {
               .hdr = MSM_CCMD(GEM_NEW, sizeof(areq)),
               .size = size,
               .flags = req.flags,
               .blob_id = blob_id,
            },
            .nr_runs = 1,
            .run = { .arena_off = aoff, .len = asize },
         };
         areq.req.iova = virtio_dev_alloc_iova(dev, size);
         if (areq.req.iova) {
            uint32_t ahandle = vdrm_bo_create(virtio_dev->vdrm, size, 0,
                                              blob_id, &areq.req.hdr);
            if (ahandle) {
               struct fd_bo *abo = bo_from_handle(dev, size, ahandle);
               struct virtio_bo *avbo = to_virtio_bo(abo);
               avbo->blob_id = blob_id;
               avbo->arena_off = aoff;
               avbo->arena_len = asize;
               abo->iova = areq.req.iova;
               return abo;
            }
            virtio_dev_free_iova(dev, areq.req.iova, size);
         }
      }
      /* fall through to legacy path */
      if (aoff) {
         simple_mtx_lock(&virtio_dev->arena_lock);
         util_vma_heap_free(&virtio_dev->arena_pool, aoff, asize);
         simple_mtx_unlock(&virtio_dev->arena_lock);
      }
   }
