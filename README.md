...
手機透過 OpenWrt 路由器 `172.22.74.2` 用 adb 連線，每台手機轉發一個 port：

| 路由器 | 手機（host） |
|---|---|
| `172.22.74.2:5566` | `192.168.66.82:5566` |
| `172.22.74.2:5567` | `192.168.66.87:5567` |
| `172.22.74.2:5568` | `192.168.66.83:5568` |

192.168.66.0/24 在路由器 NAT 後面，因此本地無法直達 192.168.66.0/24 網段，但 192.168.66.0/24 網段可以過來
不過 ipv6 部分則是全公網，可以互通。vm 也是，vm 透過 pseudo bridge 會拿到 192.168.66.0/24 的 ipv4 和公網 ipv6
...
adb -s 172.22.74.2:5566 shell "ip addr show wlan0" nodad 就是 vm ipv6，ssh key 已經設定好，直接 ssh root 連線過去

三台都有 ubuntu 26 vm，基於相同鏡像
vm 都已經內置好本地 ssh pubkey ，可直接登入 root ssh 。基於多層增量 qcow2 
目前已經建好空白增量碟，，若操作有誤，可以清除增量碟，重建空白增量碟指向原本 parent 復原狀態
