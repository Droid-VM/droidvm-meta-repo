...
| 10 | ~~simplefb udmabuf GPU copy~~ | **程式碼已收**：`63de53bfd`+`8235b4155`（一次 import 整區、fence 25ms 有界等待、任何失敗一次記錄後永久退 CPU）；`e9b295b0d`（無綁定 Stub 靜音，裝置 PASS：ERROR 0）；`ee10238db`（`transport-cap=auto\|cpu` 兩匯出端，裝置實測 cap 連 turnip 都不 dlopen）。**udmabuf-over-SHARE 前提 CONFIRMED**；WC 可見性與 §9 顏色驗收＝**tier 2 未閉**（結構上需 app 附 Surface 才有 flip）。**兩個 app 側障礙**：turnip 要 16px 倍數寬（1400 不行、1408 可以，producer 不硬編 sink 規則，CPU 退路清楚可讀）；`applyDisplayBlitEnv` 只認 gpu-0 原生（simplefb+原生拿不到 Vulkan lib）。**兩 repo 同船閘**：app 預設配置發 `transport-cap=cpu`，舊 crosvm 拒啟——APK 打包天然滿足，手動混搭會炸 |
