...
| zstd | zstd | `42f611538`（AOSP `android16-qpr2-release`） | 1b786cb3 | 1 |
...
| # | commit（功能） | meta | cross | app | edk2 | hmod | gmod | crosvm | virt | gfxs | virgl | mesa | mfst | win | vnc | zstd |
|---|---|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
...
| meta | cross | app | edk2 | hmod | gmod | crosvm | virt | gfxs | virgl | mesa | mfst | win | vnc | zstd | 合計 |
|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|
| 11 | 2 | 26 | 6 | 10 | 13 | 23 | 4 | 10 | 5 | 10 | 4 | 3 | 1 | 1 | **129** |
...
- **`external/zstd` 的 libzstd visibility 原本只存在於一台機器**，而且**直接讓 soong analysis 失敗**。
  `libzstd` 的 `visibility:` 白名單沒有 crosvm，而 visibility 由**提供者**宣告，消費者無法自我授權——
  所以一定得動 zstd 的 `Android.bp`。曾短暫做成 `soong-patches/` 的 patch overlay，後來改成
  **`Droid-VM/zstd` fork**（`droidvm` = 原封不動的 AOSP `42f611538`，我們的一個 commit 在 wip/pr 上），
  manifest 指過去、`checkout_soong` 沿分支鏈選——跟其他五個 fork 完全同形。
  `repo` 沒有 patch 機制，所以「同步完就是對的」比「同步完還要再套一次」可靠，而且那個 commit
  寫成可以直接送 AOSP 的樣子：它授權的第二個名字 `fs_mgr/libsnapshot` 根本不是我們的，
  那個相依本來就在樹裡。
