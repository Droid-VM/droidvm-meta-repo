...
**【已結：不是逐幀閃爍】** 用既有的 `POOL-SCANOUT` 資料答完了，不必再開新的量測。
...
**同一批資料還給出一個先前沒被注意到的事實**：同一顆 `res` 會被以**兩種 fourcc 都匯入過**
（`res=12 1400x1050 fourcc=0x34325241` 與 `0x34324241` 交替出現，舊樹新樹皆然）。那是
"import engaged" 事件，不是每幀事件，所以它反映的是**重新匯入時標籤會變**，而不是幀與幀之間會變。
這條線還沒查完。
