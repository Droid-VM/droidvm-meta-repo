...
**【已結：不是逐幀閃爍。但這個結論只涵蓋 guest 寫出的位元組，見下方界線】** 用既有的 `POOL-SCANOUT` 資料答完了，不必再開新的量測。

`head=` 是 **swizzle 之前的原始 guest 位元組**（`virtio_gpu.rs:715` 的註解明說 "Read the RAW guest
bytes (RGBX) before the R<->B swizzle below"），所以它直接反映 guest 寫出來的通道順序。

把 44 份 log 裡所有 1400x1050、`rgb_nonzero>0` 的記錄按 `(檔案, res)` 分組，再以 60 秒斷開成連續
區塊（塊間隔約兩分鐘，對應重開 VM）：

    連續區塊 235 個，總樣本 3854，每塊中位數 14 筆，塊內相鄰樣本間隔中位數 4.0 秒
    **塊內出現兩種位元組順序的：0 個**

**若真的逐幀 50/50 交替，每塊全同色的機率是 2^(1-n)** —— 樣本間隔 4 秒、每塊十幾筆，等於在隨機相位
上取樣十幾次。**期望全同色的塊數是 7.4，實測 235，全部。** 一個 14 筆的塊本身就給到 2^-13。

（235 這個數字是**膨脹**的：好幾份 log 是同一段 session 的重疊擷取，同一個實體區塊被重複計了。但
單一個 14 筆的塊已經足夠，結論不靠塊數。）

**所以「九輪一輪反色 ≈ 11%」這個讀法是對的**，不是「每次都在閃、只是偶爾採樣到」。出貨判準維持
「非零即阻斷」，但問題的形狀是**每次開機二選一**，不是每幀二選一。

**這個結論的界線要講清楚，因為第一版沒講**：`head=` 是 swizzle **之前**的位元組，所以它證明的是
**guest 沒有逐幀改變它寫出的通道順序**。它本身不能排除「同樣的位元組在不同 flush 被 crosvm 做了
不同處理」——那要看 fourcc 標籤在一次開機內會不會變。**下一段追了這件事，答案是不會。兩件事合起來
才構成「不是逐幀閃爍」。**

**過程中我自己製造了一個假的閃爍證據，值得記**：第一版分析按 `POOL-SCANOUT#N` 排序，而**那個編號
每次開機從 1 重來**，於是不同開機的記錄交錯成 `XoXoXoX`，看起來就是逐幀交替。**按時間戳重排之後
它立刻變成成塊的。** 一個跨 session 的序號欄位不能當時間軸用。

**追完了 fourcc 那條線，它不是新機制，是同一個已知事實的另一個角度。** 同一顆 `res=12` 確實被以
AB24 和 AR24 兩種 fourcc 匯入過，但那些 "import engaged" 事件**彼此至少相隔 1 分 41 秒，典型兩分鐘**
——正是重開 VM 的週期。**每次開機恰好一次匯入，格式在開機時決定之後整場不變。**

**中間我做出了一個假陽性，記下來因為它和前一個是同一族**：把 import 事件分進開機區間時，我用
POOL-SCANOUT 的 120 秒間隔定義開機邊界，**那個門檻把相鄰的兩次開機併成一次**，於是「一次開機兩種
fourcc」出現了六組。查時間戳才發現那六組其實是三對重疊擷取、而三對又都是同一份 log ——
**獨立觀測只有一個，而它的事件間隔說那是跨開機。** 分段門檻選錯會製造出正好是你在找的那個現象。

**所以這條線給的是一個收窄而不是一個入口**：格式是 kwin 啟動時選定的，**整場不變**，所以缺陷在
「選的時候」，不在任何中途切換。要查的是 kwin/mesa 在啟動時依據什麼選，而不是執行期什麼東西改了它。

**【後續更正：這批語料混了 route，所以它只能支撐其中一個結論】**

驗收在乾淨的單次開機邊界上量到:**fourcc 是 per-route 決定的,不是 per-boot** —— gfxstream 連續
十次開機全部 AB24、顏色全正常、水域 RGB 每次像素級相同;而 venus 和 drm2kgsl 是 AR24。

回頭查這 44 份 log:**`lc_b.txt` 的 crosvm 參數是 `context-types=virgl`,那是 venus 的 log。所以
語料確實跨 route,而且沒有任何一份 log 記錄自己的 route。** 每一份又都是跨多次開機的累積快照
(`stab/F1`–`S5` 每一份都同時含兩種 fourcc,因為每份都含多次開機)。

**因此:**

- **「不是逐幀閃爍」仍然成立。** 那個檢定問的是「連續區塊內位元組順序會不會變」,區塊內是否
  跨 route 不影響它 —— 235 個區塊零混合,無論區塊對應一次還是多次開機,都排除逐幀交替。
- **「AR24 一律 67 7d 57、AB24 兩種都有」這張提示表作廢。** 它按 `(檔案, res)` 分組,沒有按
  route 分,而 route 差異足以完整解釋它。驗收那份十次開機的資料比它乾淨得多。
- **「每次開機二選一」這個形狀失去觀測支持。** 它的來源是 fix2 那組「連續五次開機三 AB24
  兩 AR24」,而那組的 route 組成無法從既有 log 確認。
- **「九輪一輪反色 ≈ 11%」的分母有問題** —— 不同 route 的比例混在裡面。**這個數字在有
  per-route、per-boot 的乾淨樣本之前不應被引用**,包括拿它去算偵測率或規劃 N。

**共同的形狀:我按檔案分組,而檔案不是實驗單位。** 實驗單位是「一次開機 × 一條 route」,而這批
語料兩者都沒有記錄。**先前那兩個假陽性(序號當時間軸、分段門檻併開機)是同一個病的不同症狀:
用一個不是實驗單位的東西當分組鍵。**

**原本的未決問題（保留，說明它為什麼被提出）：** kwin 有三顆 scanout buffer 輪流用，**若其中一顆貼錯標籤，畫面會逐幀閃，而單張截圖只有 1/3 機率抓到壞的那一顆**。若成立，「九輪一輪反色」這個讀法整個是錯的——可能每次開機都在閃，只是偶爾採樣到，而那對出貨的意義嚴重得多。
