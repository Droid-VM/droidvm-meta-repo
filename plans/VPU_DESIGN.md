...
| **D64** | 解碼 session 與編碼 session **幾乎同時建立**時，解碼輸出**安靜地少張**：1080p+1080p 掉 30/300（同一個錯 md5 `34c34d371c…`）、1080p+720p 掉 5、4K+1080p 掉 1。`ffmpeg` 回 0、stderr 全空。**F17 的 grace 一次性沒有關掉它**（B15 仍量到 12 個 contended session 掉頭），F19 第三度定案：orchestrator 的 `DEC_CMD_START` 假說**在程式碼裡被證偽**（初次宣告時 `session.drain == None`，`decoder_cmd(START)` 整段是 no-op），真機制是 **codec 丟掉它自己收下的輸入**——`frames out` 270、`0 held output(s) dropped`，裝置一張都沒丟；ffmpeg 在宣告前排好 CAPTURE 又不再 `REQBUFS`（**D77**），宣告後沒有一個 CAPTURE buffer 借出，codec 的 21 個 output slot 全被我們扣住，它卻在 ~1.1 s 內灌完 300 個 packet，有並行 encoder 搶硬體時那批積壓就被丟掉，解碼從下一個 IDR 重來（單獨 ~50 ms 窗、被編碼器搶時 ~330 ms，所以只有宣告慢時才整個 GOP 不見） | **已知限制（D64，F20 定案，B17 再量）——F19 的 back-pressure 沒關掉它，已由 F20 移除（crosvm `c71b4b037`）**。同一個 helper 上並行且相關聯的解碼＋編碼 session 會使解碼輸出劣化：1080p 解＋1080p 編 B17 是 **0/6** bit-exact（B16 是 1/6）、4K＋1080p **0/3**、1080p＋720p **0/3**（都不是、也不預期 6/6）。**劣化維持 B16 的「混合」形狀**：B17 三次 framemd5 裡兩次保住頭 GOP、畫格對得上軟解（掉頭 GOP），一次回傳對不上任何軟解畫格的**解出來但錯**像素——**移掉 back-pressure 並沒有讓 D64 回到 B15 那種乾淨的單一掉頭 GOP 形狀**（F20-decoder §6 bar 4 猜它會，B17 證明它不會）。機制在 vendor codec 內部：裝置一張都沒丟（`0 held output(s) dropped`），編碼器單獨跑本來就 flaky——**F20 唯一移動到的是矩陣的編碼器那半，B17 每個 contended run 都 solid 300**（B16 flaky）。`KEY_LOW_LATENCY` 只在 codec 宣告 `FEATURE_LowLatency` 時設，而 `c2.qti.avc.decoder` 不宣告（每個 session 都記 `low-latency off`）；operating-rate／vendor DPB key 無法查詢、無法在 host 驗證，且會賭上今天單 session 6/6 bit-exact，所以不做。**給使用者的話：順序使用（先解完再編、或反之）乾淨；GStreamer 順序使用兩邊乾淨（DRC 20/20、encode 300/300），但併發且相關聯的 decode+encode 下 gst 也**非全免**（B14-accept-A：3 次裡 2 次 bit-exact，第三次掉 2 張）；單 session 硬解 bit-exact；並行且相關聯的 decode+encode 是已知限制。** 若日後要動，唯一剩的實驗是真機盲試 operating-rate／vendor DPB key，那要自己一個 WP、以單 session bit-exactness 為回歸門檻（B17-acceptance.md §6、F20-decoder §3）。**2026-09-19 重量（`P6c-deploy` §V4，device 已是解碼序輸出＋多 session、libva r426）**：逐位元**仍 0/12**（A 0/6、B 0/3、C 0/3，contended decoder 268–270/300）——**D64 沒關**；但**形狀變了**：18 次 framemd5 裡 **13 次是乾淨的單一 head-GOP**（hw 270 幀、只缺 0–29、0 個對不上軟解的幀、`hw[0]=sw[30]` 一路對齊），3 次 decoded-but-wrong、1 次尾段小塊——B17 說「沒回到 B15 乾淨形狀」的那件事大多回來了；4K 那格在 B17 的池子（320 MiB）下 270–299 幀、device 5.9–6.3 s（B17 268–270、9.7–11.7 s）。**未 bisect**（中間隔 D91 解碼序、fork 74a1dce/fb7c202、D85、libva r424→r426），不能歸給任何單一改動。**順帶兩個獨立發現**：(1) 這台 VM 的儲存組態把 `vpu_host_pool_mb` 明寫 **256**（App 預設 320，D68 為 4K 抬上去的）→ 4K 硬解旁邊只要有第二顆 codec 就 `media_host pool exhausted … 261529600 of 268435456 in use` → guest `REQBUFS: Cannot allocate memory`、0 幀（**不是 D64**；同組態單跑 4K 逐位元通過）；(2) 競爭時 ffmpeg CLI 自己 `Assertion pkt failed at ffmpeg_dec.c:755` SIGABRT 1/12（單跑 0/12，device 端乾淨 81 in / 21 out），D81 家族、只記錄 | codec 層內部；不是池、不是 seek/reinit、不是 `DEC_CMD_START`、也不是餵太多（back-pressure 移掉後照樣劣化） | **F20-decoder（crosvm `c71b4b037`，36 行刪除）移除 back-pressure**——它花掉單獨解碼 ~40%（B16 device 2.75–3.09 s）卻在每個失敗 run 都 engage 又擋不掉任何 loss（open item 7）。移除後 B17 單獨 1080p 解碼回到 **0.71–0.95 s device time**、6/6 bit-exact `bf32f00e5c4bca747bf7827ea5797b33`，比 B15 的 2.08 s 還快。`learned_min`／`min_capture_buffers`（D77）保留不動。D64 從此以文件化限制承載，不再當 pass/fail bar。細節見 §7.2 |
...
| **D91** | 瀏覽器（Firefox）播 **H.264 High-profile 的自適應網路串流**（MSE/HLS/DASH，含 YouTube 逼成 avc1 的 HD rendition）**軟解退回**：每個解碼 session 只餵約 3 個 access unit 就卡在第一次 `vaSyncSurface`，而 `c2.qti.avc.decoder` 對 High profile **要先解出約 6 張圖像才發 `SOURCE_CHANGE`**（Constrained Baseline 只要 1 張）；grace 到期釋放已餵輸入 → sequence-1 sync 失敗 → `vaExportSurfaceHandle failed` → 退軟解（B23/B25：裝置 3 進 0 出、grace 觸發、0 fmtΔ） | **關（2026-09-18，`D91-h264` → `D91-lowlat-probe` → `D91-crosvm-impl` → `D91-deploy`）——兩個成因，兩個都是我方的，B23→B29 的「已知限制」是誤判。** (1) **那個「6 張」是我們寫的**：合成 SPS 的 VUI `max_num_reorder_frames` 填了 `num_ref_frames`（真實 High 串 5，原生 2），合規解碼器到第 6 個 AU 才吐第一張（6 = 5 + 1）；改 0（libva-v4l2 `b113ac7`，§7.6 第 3 點），第一張提前到 AU 1，串本身以獨立軟解 gate 證明有效。這一步之後 Firefox 真的拿到 5 張零拷貝硬解幀（8 in / 5 out），然後撞第二個成因。(2) **`c2.qti.avc.decoder` 預設吐顯示序、為重排扣圖**：探針量到 self-delay（餵完某 AU 之後還要再餵幾個它自己那張才回來）**p95 = 7、3 張不靠 EOS 永遠不回**，而 Firefox 的 feed-ahead 預算是 7–8 個 AU（`D85-drain` §3.3）——正好卡在邊界，等待自己剛送的那張＝§7.6 第 5 點的重排死鎖，門檻掃 50/1000/3000/8000 ms 逐位元相同證明它不是誤判 EOS。這顆 codec 對 AVC 公開 **`vendor.qti-ext-dec-picture-order.enable`**：configure 時設 1 → 解碼序輸出，self-delay p50 = p95 = 1、`nonmono` 157 → 0、EOS 尾巴 3 → 0，逐幀 md5 依 PTS 對齊 300/300 完全相同（`.low_latency` 變體與 `KEY_LOW_LATENCY` 對它一格都沒改，且變體 bitrate 上限砍到 70 Mbps，純虧）。**修在 crosvm**（fork `9d6fe4983`）：只對 `video/avc` 在 configure 時設這個 key（`start()` 之後的 `setParameters` 是 no-op），`start()` 後從 **OUTPUT** format 回讀（component 不在 input format 回音——`VA3-mcmatrix` 讀錯 port 才把它記成不存在）、ledger 印 `picture-order on|off`。**實機（`D91-deploy`，App 路徑 crosvm md5 `002659f6…`，出貨組態）**：ledger AVC 全 on、VP9/AV1 全 off 零例外；三條 H.264 canonical clip **逐位元相同**（解碼序交付之下 `session.cc` 依 timestamp 認領仍正確——這是唯一沒在真 VA 路徑驗過的推論，已證）且 idle drain 0/0/0（原 0/1/1）；VP9/AV1 三條逐位元不變；browser-free `-flags low_delay` 重現 1800/1800、0 drain、0 syncfail（原 8 AU、2 drain、1 syncfail）；**出貨 Firefox MSE adaptive High 單一 session 9000 in / 9000 out、150 s**，idle drain 0、syncfail 0、`gbm-dmabuf`、無軟解（原 8 in / 5 out、2.13 s 後退軟解）；**真 YouTube 強制 avc1 10504 / 10504、175 s**；`h264_v4l2m2m` 300/300 bit-exact | **我方兩處**：libva-v4l2 的 VUI 欄位（重建缺陷）＋ crosvm 的 configure（沒有向 codec 要解碼序）。**不是** Qualcomm 的門檻、**不是** Firefox 的餵料深度、不是 device／driver／池 | 已落地：libva `b113ac7`（wip/vpu）；crosvm `9d6fe4983`（fork wip/vpu，已部署）。殘留：HEVC 沒有 clip 可驗 mime gate；AV1 深-B 值得用正確的 output-port readback 重測（VA2l「兩條槓桿雙雙用盡」的前提是讀錯 port）；P-6b（連續換影片頁時 session 交疊、靜默退軟解）另開。**教訓**：codec 的行為要用探針量、readback 要讀對 port、重建的位元流要先過獨立軟解 gate——三件事任何一件早做，B23→B29 六輪都不用跑。證據 `logs/vpu_wp/D91-*.md`、`D85-drain.md`
...
5. **重排死鎖規則。** VA 客戶端自己重排、按顯示順序取畫格，**拿到 S 之前不會再送輸入**；stateful 解碼器若扣住
   比客戶端更多的畫格，雙方互等。**對 AVC，這個成因已從根移除（2026-09-18，D91）**：crosvm 在 configure 時對
   `c2.qti.avc.decoder` 設 `vendor.qti-ext-dec-picture-order.enable=1`，codec 吐**解碼序**、不為顯示重排扣圖
   （self-delay p95 7 → 1，EOS 尾巴 3 → 0），而這個後端的 CAPTURE 本來就依 timestamp 認領、客戶端自己重排，所以
   「codec 扣得比客戶端多」在 AVC 上不再成形——三條 H.264 clip 逐位元不變、idle drain 全 0、Firefox MSE 9000/9000。
   VP9／AV1 沒有這個 key（AV1 值得用正確的 output-port readback 重測），下面的規則與兩層等待仍然是它們的保險。
   兩道保險：(a) 第 3 點的 VUI（宣告 0 的重排深度），讓 codec 的延遲不超過客戶端的假設；(b)
   ...
   * **idle drain**（B18 之後新增）——客戶端在等、而 device 一段時間**沒有任何新輸出**（沒有東西在飛），
     ...
     **這是 EOS 的替代品**：VA-API **沒有 flush／EOS 呼叫**（第 6 點只在 `vaDestroyContext` 排空），所以帶
     ...
     bit-exact**。所以帶 B-frame 的片子**預期正好一次**，不是缺陷（**D85**）——**AVC 開解碼序之後是 0**（`D91-deploy`：EOS 尾巴不再被扣住，走不到排空）。
...
**D91 已關（2026-09-18；原本這一段記的「已知限制」是誤判）。** stock Firefox 現在對 H.264 High-profile 的自適應網路串流（MSE/HLS/DASH、YouTube 逼成 avc1）也零拷貝硬解：出貨組態、App UI、零環境變數，單一 session **9000 in / 9000 out（150 s）**、YouTube avc1 **10504 / 10504（175 s）**、0 次排空、0 次 sync 失敗。兩個成因都在我方：合成 SPS 的 VUI 重排深度填了 `num_ref_frames`（那就是「codec 要 6 張」的 6 = 5 + 1，§7.6 第 3 點），以及 crosvm 沒有向 `c2.qti.avc.decoder` 要**解碼序輸出**（`vendor.qti-ext-dec-picture-order.enable`，§7.6 第 5 點）。「Qualcomm 的不可設定門檻」與「Firefox 的餵料深度」都不成立，也不需要 Chromium 的 V4L2-native 路。全紀錄在 §7.4 D91 列與 `logs/vpu_wp/D91-h264.md`／`D91-lowlat-probe.md`／`D91-crosvm-impl.md`／`D91-deploy.md`。
...
**目前的部署版本（2026-09-18，手機 App 路徑上跑的就是這一組）。** crosvm fork `wip/vpu` **`9d6fe4983`** ＋ virtio-media fork `wip/vpu` **`fb7c202`**（App 路徑 md5 `d602850a421bba88b66339a9989489e3`，`P6c-deploy` 部署；前幾版 `ca29fec1…`／`002659f6…`／`6b1b8c41…` 留在 `/data/local/tmp/crosvm_*_backup_*`；含 P-9 修法的 `310eb7f9` 已建、待部署）、libva-v4l2 **r426 `ad0492f`**（`.so` md5 `bf9aa557f4e0a03a5ea8eab429388b19`；wip/vpu `26eb183` 的樹與它相同）、mesa fork `wip/vpu` **`4b23362ebfe`**（`mesa-guest` r227673，zink 那一行）、guest-additions **r25**。guest-additions **r26 `329503a`**（P-1 的 greeter 備援底色 drop-in，§7.8；`P1-verify` 在新增量碟上驗過）。
...
| **P-6b** | 連續換影片頁（vp9.html → av1.html）或 `<video loop>` 重播時**靜默退軟解**：device 不開 session、journal 停在 `gbm:` 探測；三個 WP 各踩一次 | **關（2026-09-19，`P6b-verify` → `P6b-device` → `P6b-deploy`），兩層、兩個都是我方的。** (a) **libva**：所有 stateful context 共用 `vaInitialize` 開的那一個 fd（一個 open = 一個 codec session），Firefox 的 decoder 共用一個 VADisplay、又在舊 decoder 收尾前先建新的 → 第二個 `vaCreateContext` 撞 `VIDIOC_S_FMT(OUTPUT): Device or resource busy`；修法 libva `ad0492f`（wip/vpu `26eb183`，r426）：每個 context 自己 `open()` 節點 → `<video loop>` 交接 0/1 → 15/15、53 ms，fd 播放中 2、關頁後 0。(b) **device**：virtio-media fork `video_decoder.rs` 的 `active_session` 閘（「一次只服務一個 session，第二個 `REQBUFS`/`STREAMON` 回 EBUSY」，註解說是為了 v4l2-compliance——**但 compliance 那個檢查包在 `if (!node->is_m2m)` 裡，從來沒跑在這顆 M2M decoder 上**），而 Firefox 換頁時把舊 decoder 多抱 2.4–5.6 s > libva 2000 ms 的 provisioning 預算 → 新頁的 `REQBUFS(OUTPUT)` EBUSY 到放棄。`P6b-verify` 一度把它歸成「手機一次只服務一個 decoder」，兩個獨立 process 也重現——**錯的是我們的欄位，不是手機**。修法 fork `74a1dce`：拿掉閘，session 各自的 codec／queue／buffer／事件本來就獨立，上限只剩池子（ENOMEM）與 codec store（EBUSY 經 backend）；vmt 160/0。**實機（crosvm `ca29fec1…`）**：65 個 session、EBUSY 0、`Failed to create context` 0、重試 0；七條 canonical 位元精確；同一 VADisplay 兩路／兩個 process／三個 client 同時上**全部位元精確且時間重疊**；5 輪並行壓力 10/10（`held output(s) dropped` 0，D64 那種掉幀在 decode＋decode 上沒出現）；出貨 Firefox **直接**換頁 vp9→av1→h264→vp9 四頁四 session 零退軟解、loop 12/12、MSE 9000/9000、YouTube AV1 14854/14854、**兩支影片同時硬解**（fd = display + 2）；encoder 300/300 | libva-v4l2（已落地、已推）＋ virtio-media fork device `74a1dce`（decoder，已推）＋ **`fb7c202`（encoder 的同型閘也拿掉，vmt 161/0；`P6c-deploy`：兩個／三個 encoder 同時 300/300、重疊 11 s，running codec 的 S_CTRL EBUSY 規矩不變，四條 canonical 位元精確、Firefox 直接換頁零退軟解）**。D64 重量見 §7.4 |
...
| **P-9** | **任何開音效的 VM 跑 12.4 小時就整台崩**：閒置的 VM 在 uptime 12.43 h 時 `thread 'main' panicked at android_audio/src/lib.rs:740: attempt to add with overflow` → snd helper 死 → `exiting with crash`（`P6c-deploy` 第一次嘗試留著的 VM，04:16 起、16:42 崩）| **修在 crosvm fork（`31182f706`，未推、未部署）**：`AudioStream::total_frames` 是 `i32`，48 kHz 下 2³¹/48000 s = 12.43 h 溢位，而 release profile 開 `overflow-checks = true`、`panic = abort`；改 `u64`（deadline 的 `×1000` 在 48 kHz 下約 12 000 年才溢位），pacing 語意不變；三個單元測試（種子 `i32::MAX-1` 跨過 2³¹）在 host 上 `cargo test -p android_audio --features libaaudio_stub` 12/12，改回 i32 就在同一行 panic 出同一句。已建成 crosvm `310eb7f9`（= `d602850a` ＋ 此修法），待 smoke ＋ 過夜 soak。同型風險 `read_count: i32`（capture，10 ms period ≈ 248 天）未動；`libaaudio_stub.rs` 少 7 個符號暫放測試模組，待搬 | crosvm fork `android_audio`。**設計註記**：host 端任何 helper（snd／media）死掉，VMM 都當致命錯誤、整台 VM 陪葬——guest→host 的 session 生命週期由 kernel fd 保證不脫鉤，host→guest 方向沒有降級路徑，待決 |
